...
const CheckoutForm: React.FC = () => {
  ...
  const { items } = useCart(); // Get cart items from the context
      ...
      <div className="flex flex-col gap-5 lg:flex-row">
        <div className="flex flex-col gap-y-5 w-full lg:w-1/2 lg:pr-4">
          ...
          <div className="mt-4">
            ...
            <textarea
              placeholder="Notes about your order( any special notes)"
              className="border border-[#501B1F] bg-[#F2EEED] rounded-[9px] w-full h-[100px] p-3 placeholder:text-[#12180A] placeholder:text-opacity-60"
            ></textarea>
          </div>
        </div>
        <div className="w-full lg:w-1/2 lg:pl-4">
          ...
          <div className="bg-[#FFEFF0] border border-[#7F5759] p-4 mb-6">
            <div className="flex flex-col gap-y-5 text-[#180A0B] border-b border-[#F2E2E2] pb-6 mb-6">
              ...
              <ul>
                {items.map((item) => (
                  <li key={item.id} className="lg:text-lg flex items-center justify-between gap-x-4">
                    <p>{item.name} | {item.quantity} {item.quantity > 1 ? 'Bottles' : 'Bottle'}</p>
                    <p className="text-right text-[#910B15]">NGN{item.quantity * 8000}.00</p>
                  </li>
                ))}
              </ul>
              <div className="font-bold flex items-center justify-between gap-x-4">
                <h4>Subtotal</h4>
                <h4 className="lg:text-lg text-right">NGN{items.reduce((acc, item) => acc + item.quantity * 8000, 0)}.00</h4>
              </div>
            </div>
          ...
          <div>
            ...
            <div>
              <div className="flex gap-x-4 items-center mt-2">
                <input type="checkbox" id="other" />
                <label className="text-[#180A0B]">
                  I agree to the{" "}
                  <a className="underline text-[#501B1F]">Terms and Conditions</a>
                </label>
              </div>
            </div>
          </div>
